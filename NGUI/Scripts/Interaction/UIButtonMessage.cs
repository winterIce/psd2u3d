	...
	void Send ()
	{
		...
		if (includeChildren)
		{
			...
			for (int i = 0, imax = transforms.Length; i < imax; ++i)
			{
				...
				t.gameObject.SendMessage(functionName, parameterString, SendMessageOptions.DontRequireReceiver);
			}
		}
		else
		{
			target.SendMessage(functionName, parameterString, SendMessageOptions.DontRequireReceiver);
		}
	}
}